...
class DeepSeekProvider:
    ...
    def __init__(self, api_key: str, api_url: str = "https://api.deepseek.com/v1/chat/completions"):
        ...
        Args:
            ...
            api_url: API endpoint URL (default: DeepSeek chat completions endpoint)
        """
        self.api_key = api_key
        self.api_url = api_url
        if not self.api_key:
            raise ValueError("DeepSeek API key is required. Set it in config.json or DEEPSEEK_API_KEY environment variable.")
    ...
    def call(self, model: str, prompt: str, system_prompt: str = "", temperature: float = 0.7, top_p: float = 0.9, max_tokens: int = 4096) -> str:
        ...
        try:
            ...
            if "choices" not in result or len(result.get("choices", [])) == 0:
                print(f"WARNING: DeepSeek API response has no choices. Full response: {json.dumps(result, indent=2)}")
            ...
            if finish_reason != "stop":
                print(f"WARNING: DeepSeek API finish_reason: {finish_reason}. This may indicate why response is empty.")

            if "message" not in choice:
                print(f"WARNING: DeepSeek API choice has no message. Choice: {json.dumps(choice, indent=2)}")
                return ""

            content = choice["message"].get("content", "")

            if not content or not content.strip():
                print(f"WARNING: DeepSeek API returned empty content (finish_reason: {finish_reason})")
            ...
            error_msg = f"DeepSeek API request failed: {e}"
